...
--[[
	MapBuilder
	Open-floor facility map — one big walkable building, no sealed box rooms.
	Only outer walls + one interior door divider. Guaranteed connected.
]]
...
local B = {
	...
	MinZ = -70,
	MaxZ = 80,
	DoorZ = 42, -- interior divider between front and back
}
...
function MapBuilder.build()
	...
	-- === OPEN FLOOR ZONES (all connected, no interior box walls) ===
	...
	-- Colored accent zones so rooms are visible (flat on floor, no walls)
	...
	-- ONE interior wall — divider with doorway to back section
	...
	-- Objectives
	key(objectives, Vector3.new(-30, 5, 8)) -- Office zone
	fuelCell(objectives, "FuelCell1", Vector3.new(-30, 2, 30)) -- Storage
	fuelCell(objectives, "FuelCell2", Vector3.new(30, 2, 15)) -- Cafeteria
	generator(objectives, Vector3.new(-30, 3.5, 38)) -- Generator zone
	fuelCell(objectives, "FuelCell3", Vector3.new(0, 2, 60)) -- Back section
	door(objectives, Vector3.new(0, 6, B.DoorZ)) -- Main door in divider
	...
	sign(structure, "WALK NORTH", Vector3.new(0, 8, -15))
